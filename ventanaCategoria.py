...
class VentanaCategoria:
    ...
    def agregarCategoria(self):
        ...
        try:
            resultado = self.db.create_categoria(nombre)
            
            if resultado[0]:
                categoriaId, mensaje = resultado
                messagebox.showinfo("Éxito", mensaje)
                self.nombreVar.set("")
                self.loadCategorias()
            else:
                categoriaId, mensaje_error = resultado
    ...
    def guardarEdicion(self):
        ...
        try:
            resultado = self.db.update_categoria(self.editingId, nombre)
